...
final readonly & map<map<GrowthRange>> weightTables = {
    "male": {
...
isolated function getDefaultVaccines(string gender) returns VaccineRecord[] {
...
service /health on new http:Listener(9090) {
    ...
    // Signup for health records
        isolated resource function post signup(@http:Payload SignupRequest signupReq) 
    ...
    isolated resource function post login(@http:Payload record {
        string email;
        string password;
...
// - `post addDisease`: Adds a new disease record to the `diseases` collection in MongoDB.  
  isolated resource function post addDisease(@http:Payload DiseaseRecord disease)
...
// - `get getDiseases`: Fetches all disease records of a user (by `userId`) from MongoDB. 
    isolated resource function get getDiseases(@http:Query string userId)
...
isolated resource function get getVaccines(@http:Query string userId)
...
isolated resource function put markVaccineReceived(@http:Payload record {
    string userId;
    string vaccineName;
    string dose;
    string receivedDate;
...
isolated resource function post addCustomVaccine(@http:Payload record {
    string userId;
...
isolated resource function post checkGrowth(@http:Payload record {
    string userId;
    float weight;
    float height; // in cm
...
isolated resource function get getUserProfile(@http:Query string userId) returns http:Ok|http:NotFound|error {
    mongodb:Database db = check mongoClient->getDatabase("healthDB");
    mongodb:Collection usersCollection = check db->getCollection("users");
    ...
    string firstName = userDoc["firstName"] is string ? <string>userDoc["firstName"] : "";
    string lastName = userDoc["lastName"] is string ? <string>userDoc["lastName"] : "";
    string email = userDoc["email"] is string ? <string>userDoc["email"] : "";
    string gender = userDoc["gender"] is string ? <string>userDoc["gender"] : "";
    string phoneNumber = userDoc["phoneNumber"] is string ? <string>userDoc["phoneNumber"] : "";
    string dateOfBirth = userDoc["dateOfBirth"] is string ? <string>userDoc["dateOfBirth"] : "";
...
isolated resource function get getDocAppointments(@http:Query string userId) returns http:Ok|http:NotFound|error {
    mongodb:Database db = check mongoClient->getDatabase("healthDB");
    mongodb:Collection docAppointmentsCollection = check db->getCollection("doc_appointments");

    stream<record {}, error?> resultStream = check docAppointmentsCollection->find({ "userId": userId });
    record {}[] appointments = [];
